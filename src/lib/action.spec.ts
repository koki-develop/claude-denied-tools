...
import { _extractDeniedTools, _getIssueNumber, _renderReports } from "./action";
...
_Generated by [koki-develop/claude-denied-tools](https://github.com/koki-develop/claude-denied-tools)_
`.trim(),
    );
  });
...
describe("_getIssueNumber", () => {
  it("should return pull_request number", () => {
    const context = {
      eventName: "pull_request",
      payload: { pull_request: { number: 42 } },
    } as unknown as Context;
    expect(_getIssueNumber(context)).toBe(42);
  });

  it("should return issue number for issue_comment event", () => {
    const context = {
      eventName: "issue_comment",
      payload: { issue: { number: 7 } },
    } as unknown as Context;
    expect(_getIssueNumber(context)).toBe(7);
  });
  ...
  it("should return null for push event", () => {
    ...
    expect(_getIssueNumber(context)).toBeNull();
  });

  it("should return null for workflow_dispatch event", () => {
    const context = {
  ...
  it("should return null when pull_request number is missing", () => {
    const context = {
      eventName: "pull_request",
